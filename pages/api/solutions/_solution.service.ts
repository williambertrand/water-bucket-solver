...
const getStepsForBigToSmall = (params: SolutionParams): Step[]  => {
  ...
  while(bigVal !== c) {
    ...
    if(bigVal == 0 ) {
      steps.push({ type: 'FILL', display: `Fill bucket ${bigBucket}` })
      bigVal = bigBucketSize;
    }

    steps.push({ type: 'TRANSFER', target: bigBucket, display: `Transfer from ${bigBucket} to ${smallBucket}`})
    ...
    if(smallVal == smallBucketSize && bigVal !== c) {
      steps.push({ type: 'EMPTY', target: smallBucket, display: `Empty ${smallBucket}`})
      smallVal = 0
    }
  }

  steps.push({ display: `Complete! You now have ${c} units of water in bucket ${bigBucket}`, type: 'COMPLETE', target: bigBucket })
...
const getStepsForSmallToBig = (params: SolutionParams): Step[]  => {
  ...
  while(bigVal !== c) {
    if(smallVal == 0 ) {
      steps.push({ type: 'FILL', display: `Fill bucket ${smallBucket}` })
      smallVal = smallBucketSize;
    }

    steps.push({ type: 'TRANSFER', target: smallBucket, display: `Transfer from ${smallBucket} to ${bigBucket}`})
    ...
    if(bigVal === bigBucketSize) {
      steps.push({ type: 'EMPTY', target: bigBucket, display: `Empty ${bigBucket}`})
      bigVal = 0
    }
  }
